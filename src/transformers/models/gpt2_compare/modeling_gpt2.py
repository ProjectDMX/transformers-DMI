# GPT-2 COMPARE variant: identical to gpt2_p but also captures
# tensors via .copy_() to pre-allocated buffers for bitwise comparison
# against ClickHouse output. Used for transport correctness testing.
# coding=utf-8
...
class GPT2MLP(nn.Module):
    ...
    def forward(self, hidden_states: Optional[tuple[torch.FloatTensor]]) -> torch.FloatTensor:
        ...
        self._buf_mlp_post[:hidden_states.shape[0], :hidden_states.shape[1]].copy_(hidden_states)
...
class GPT2Block(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        self._buf_attn_out[:attn_output.shape[0], :attn_output.shape[1]].copy_(attn_output)
        ...
        self._buf_mlp_out[:feed_forward_hidden_states.shape[0], :feed_forward_hidden_states.shape[1]].copy_(feed_forward_hidden_states)
        # residual connection
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        self._buf_pos_embed[:position_embeds.shape[0], :position_embeds.shape[1]].copy_(position_embeds)
...
class CompareGPT2Model(GPT2Model, HookedRootModule):
...
class CompareGPT2LMHeadModel(GPT2LMHeadModel, HookedRootModule):
    ...
    def get_hook_specs(self) -> list:
        ...
        for i, block in enumerate(tr.h):
            ...
            specs.append(HookSpec(HOOK_TYPE_K,          block.attn.hook_k, layer_no=i))
            specs.append(HookSpec(HOOK_TYPE_V,          block.attn.hook_v, layer_no=i))
            specs.append(HookSpec(HOOK_TYPE_Q,          block.attn.hook_q, layer_no=i))
            if is_eager:
    ...
    def allocate_compare_buffers(
        ...
        hd = H // nh
        I = config.n_inner if config.n_inner is not None else 4 * H
        V = config.vocab_size
        B, S = batch_size, max_seq_len
        device = "cuda"

        tr = self.transformer
        tr._buf_embed = torch.empty(B, S, H, device=device, dtype=dtype)
        tr._buf_pos_embed = torch.empty(B, S, H, device=device, dtype=dtype)
        tr._buf_resid_final = torch.empty(B, S, H, device=device, dtype=dtype)
        tr._buf_final_ln = torch.empty(B, S, H, device=device, dtype=dtype)

        for block in tr.h:
            attn = block.attn
            block._buf_resid_pre = torch.empty(B, S, H, device=device, dtype=dtype)
            block._buf_ln1 = torch.empty(B, S, H, device=device, dtype=dtype)
            block._buf_attn_out = torch.empty(B, S, H, device=device, dtype=dtype)
            block._buf_resid_mid = torch.empty(B, S, H, device=device, dtype=dtype)
            block._buf_ln2 = torch.empty(B, S, H, device=device, dtype=dtype)
            block._buf_mlp_in = torch.empty(B, S, H, device=device, dtype=dtype)
            block._buf_mlp_out = torch.empty(B, S, H, device=device, dtype=dtype)
            block.mlp._buf_mlp_post = torch.empty(B, S, I, device=device, dtype=dtype)
            attn._buf_q = torch.empty(B, S, nh, hd, device=device, dtype=dtype)
            attn._buf_k = torch.empty(B, S, nh, hd, device=device, dtype=dtype)
            attn._buf_v = torch.empty(B, S, nh, hd, device=device, dtype=dtype)
