# Hooked Qwen3 COMPARE variant: identical to qwen3_p but also captures
...
class Qwen3MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.hook_post(x)
        self._buf_mlp_post[:x.shape[0], :x.shape[1]].copy_(x)
        x = self.down_proj(x)
        return x
...
class CompareQwen3Model(Qwen3Model, HookedRootModule):
...
class CompareQwen3ForCausalLM(Qwen3ForCausalLM, HookedRootModule):
    ...
    def _normalize_hook_names(self) -> None:
        """Drop `model.` prefix so names match `CompareQwen3Model`."""
